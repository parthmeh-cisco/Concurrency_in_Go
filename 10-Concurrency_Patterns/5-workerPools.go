...
func main() {
	...
	jobsChan := make(chan int, numJobs)          // creates a buffered channel large enough to hold all the jobs
	completedJobsChan := make(chan int, numJobs) // creates a buffered channel large enough to hold all the completed jobs

	for w := 1; w <= 3; w++ { // this is the number of workers. Each worker will be a goroutine.
		go worker(w, jobsChan, completedJobsChan) // create worker w - pass worker number and both channels
	}

	for j := 1; j <= numJobs; j++ {
		jobsChan <- j // This loads the jobsChan channel with the job numbers
	}

	close(jobsChan) // Close the jobsChan channel for input after all jobs have been loaded. Channel must be closed in order to call "range" function.
